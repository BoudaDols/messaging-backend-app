...
function validateEmail(email) {
	...
	if (!emailRegex.test(trimmed)) {
		return {
			valid: false,
			error:
				"Email must contain exactly one @ symbol followed by a domain with at least one dot",
		};
	}
...
function validatePassword(password) {
	...
	if (!strongPasswordRegex.test(password)) {
		return {
			valid: false,
			error:
				"Password must contain at least one uppercase letter, one lowercase letter, one number, one special character, and a minimum length of 8 characters.",
		};
	}
...
function validateDisplayName(displayName) {
	...
	if (displayName.trim().length === 0) {
		return {
			valid: false,
			error: "Display name must contain at least one non-whitespace character",
		};
	}
